...
VALUES
   ...
   -- Basic Query --
-- Number 1 --
	SELECT c.cst_name, m.mbr_points
	FROM customer c JOIN membership m ON c.cst_id = m.customer_cst_id
	WHERE m.mbr_points > 100 AND m.mbr_points < 115;
...
-- Number 3 --
  	SELECT customer_cst_id AS 'costumer id', AVG(tr_totalBill) AS average_total_bill
  	FROM TRANSACTION
  	GROUP BY customer_cst_id
  	HAVING average_total_bill > 5000000;
  
-- Number 4 --
 	SELECT tr_paymentMethod AS 'Payment method', count(*) AS 'Total'
 	FROM transaction 
 	WHERE tr_paymentMethod = "cash";
 		 
-- Number 5 --	
	SELECT c.cst_id, c.cst_name, c.cst_phoneNumber, m.mbr_email, COUNT(*) AS total_transactions
	FROM customer c 
		JOIN transaction t ON t.customer_cst_id = c.cst_id 
		JOIN membership m ON m.customer_cst_id = c.cst_id
	GROUP BY t.customer_cst_id, m.mbr_email
...
-- Number 6 --
	SELECT AVG(tr_totalBill) AS average_total_bill, t.customer_cst_id FROM transaction t
	GROUP BY t.customer_cst_id
	HAVING AVG(tr_totalBill) > (SELECT AVG(tr_totalBill) FROM TRANSACTION)
	ORDER BY AVG(tr_totalBill) DESC;
	
-- Number 7 --
	WITH e AS (
    SELECT employee_emp_id, COUNT(*) AS Total
    FROM transaction
    GROUP BY employee_emp_id
	)
	SELECT emp_name, emp_id, Total AS total_transaction 
	FROM e 
		JOIN employee em ON e.employee_emp_id = em.emp_id
	WHERE Total = (SELECT MAX(Total) FROM e);
	
-- Number 8 --
	SELECT c.cst_id, c.cst_name, SUM(tr_totalBill) AS total_bill
	FROM transaction t 
		JOIN customer c ON c.cst_id = t.customer_cst_id
	GROUP BY customer_cst_id
	HAVING SUM(tr_totalBill) > (SELECT AVG(tr_totalBill) FROM transaction);
	
-- Number 9 --
	SELECT c.cst_name, c.cst_id, COUNT(*) AS total_transaction
	FROM TRANSACTION t 
		JOIN customer c ON c.cst_id = t.customer_cst_id 
	GROUP BY t.customer_cst_id
...
-- Number 10 --
	SELECT i.*
	FROM item i
		LEFT JOIN transaction_item t ON t.item_item_id = i.item_id
	WHERE t.item_item_id IS NULL;